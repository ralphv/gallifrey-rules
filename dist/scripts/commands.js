...
function validateCommand(name) {
    if (!(name in exports.commands)) {
        throw new Error(`Command ${name} is not a valid command.`);
    }
    const variablesExist = exports.commands[name]
        .filter((a) => !a.optional)
        .every(({ environmentVariables }) => environmentVariables.some((a) => a in process.env));
    if (variablesExist) {
        return true;
    }
    const missing = exports.commands[name]
        .filter((a) => !a.optional)
        .find(({ environmentVariables }) => environmentVariables.every((a) => !(a in process.env)));
    console.warn(`One or more environment variables are missing: ${JSON.stringify(missing.environmentVariables)}`);
    const eVariables = exports.commands[name]
        .map((envVar) => `-e ${envVar.environmentVariables.length === 1 ? String(envVar.environmentVariables[0]) : "'" + envVar.environmentVariables.join(' OR ') + "'"}='< ${envVar.description}${envVar.optional ? ' [OPTIONAL]' : ''} >'`)
        .join(` \\${node_os_1.EOL}    `);
    const command = `
docker run -it --rm \\
